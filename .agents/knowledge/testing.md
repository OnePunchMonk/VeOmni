...
- Adding a file under `tests/ops/` or `tests/data/` is already covered. Adding a
  workflow line for it is redundant churn.
- Adding a file anywhere else is **invisible to CI** until it is enumerated —
  usually in two workflows. A large number of existing files under
  `tests/{trainer,utils,models,lora,checkpoints,distributed,parallel}` are in
  exactly this state, so "a test file exists for X" does not mean X is guarded.
...
`tests/ops/` or `tests/data/`.** Covered automatically; do not touch workflows.
...
- Changes whose only failure mode is a build/lint error that another gate
  already catches (`check_patchgen`, `check_docker_generate`, `device_api_check`,
  ruff).
